...
class MillenniumDate {
    ...
    static fromNow(offsetHours) {
        let now = new Date();
        if (offsetHours !== undefined)
            return new MillenniumDate(now.getTime(), offsetHours);
        else
            return new MillenniumDate(now.getTime(), (-now.getTimezoneOffset() / 60));
    }
    static fromDate(date, offsetHours) {
        if (offsetHours !== undefined)
            return new MillenniumDate(date.getTime(), offsetHours);
        else
    ...
    getHourOfDay24Hour(zeroIndexed = false) {
        let beginningOfDay = (new millennium_date_builder_1.MillenniumDateBuilder())
            .withYear(this.getYear())
            .withMonth(this.getMonth())
            .withDay(this.getDayOfMonth())
            .build();
        let zeroIndexedResult = Math.floor(millennium_timespan_1.MillenniumTimespan.between(beginningOfDay, this).toHours());
        if (zeroIndexed)
            return zeroIndexedResult;
        else {
            if (zeroIndexedResult === 0)
                return 12;
            else
                return zeroIndexedResult + 1;
        }
    }
    getHourOfDay12Hour() {
    ...
    toString(prefer24Hour = false) {
        let dayOfWeek = millennium_week_day_1.MillenniumWeekDay.getViaDate(this).getName();
        ...
        let hour = (prefer24Hour ? this.getHourOfDay24Hour().toString() : this.getHourOfDay12Hour().toString());
